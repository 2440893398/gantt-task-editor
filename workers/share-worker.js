...
import feedbackBoardPageTemplate from './feedback-board-page.html.txt';
import {
...
/**
 * 允许被回显的跨域来源（代码评审 2026-09-02 §5.4）。
 *
 * 配置来源：`FEEDBACK_PRODUCTION_ORIGIN`（Pages 站点，工作台的真实调用方）与可选的
 * `FEEDBACK_ALLOWED_ORIGINS`（逗号分隔，给 CN 站点 / 预览域名这类额外部署用）。
 * 本机开发的 localhost / 127.0.0.1 任意端口一律放行——否则 `npm run dev` 与
 * workbench E2E 全部撞墙。
 */
function feedbackAllowedOrigin(requestOrigin, env) {
    ...
    const configured = [
        String(env?.FEEDBACK_PRODUCTION_ORIGIN || ''),
        ...String(env?.FEEDBACK_ALLOWED_ORIGINS || '').split(','),
    ]
        .map((value) => value.trim().replace(/\/+$/, ''))
        .filter(Boolean);
    return configured.includes(parsed.origin) ? parsed.origin : '';
}

/**
 * `requestOrigin`/`env` 此前是**死参数**（§5.4）：函数签名零参，而调用点一直在传
 * `request.headers.get('Origin')`，被静默丢掉——全站（含管理 API）一律
 * `Access-Control-Allow-Origin: *`。
 *
 * 现在允许清单内的来源被逐字回显（并带上 `Vary: Origin`，否则中间缓存会把一个来源
 * 的响应喂给另一个）。**清单外仍然回 `*`**：本仓不止一个前端部署（CN 站点、Pages
 * 预览域名），把未知来源改成拒绝是一次部署决策而不是代码决策，需要先盘清来源清单；
 * 而这里的授权全部走 `Authorization` 头、不依赖 cookie，`*` 不构成环境权限外泄。
 * 这条取舍写在这里，是为了让「为什么还是 `*`」有据可查，而不是像之前那样看起来
 * 像忘了接线。
 */
function corsHeaders(requestOrigin = '', env = null) {
    const allowed = feedbackAllowedOrigin(requestOrigin, env);
    return {
...
/**
 * legacy board 页面（代码评审 2026-09-02 §5.12）。
 *
 * 这里原本内联着约 2000 行 HTML/CSS/JS 模板字符串——share-worker.js 之所以是
 * 13.4k 行，这一块占了七分之一。V2 工作台早就示范了正确形态（`.txt` 资产 +
 * 独立模块，见 feedback-workbench-ui.js），这次把 legacy 页面搬到同一条路上。
 *
 * 服务端只注入 11 个值，全部走 `__TOKEN__` 占位符替换；页面内的
 * `${...}` 与反引号都是**浏览器端**的模板字面量，留在资产里原样交付
 * （内联时它们必须逐个反斜杠转义，这也是这段模板极难读的原因之一）。
 */
function renderFeedbackBoardPage(apiBase = '') {
    ...
    const substitutions = {
        __REPLAY_STYLE_PATH__: FEEDBACK_REPLAY_STYLE_PATH,
        __REPLAY_SCRIPT_PATH__: FEEDBACK_REPLAY_SCRIPT_PATH,
        __SUBMITTED_TYPE_OPTIONS__: submittedTypeOptions,
        __STATUS_OPTIONS__: statusOptions,
        __PRIORITY_OPTIONS__: priorityOptions,
        __FEEDBACK_API_BASE__: feedbackApiBase,
        __LIMIT_TITLE__: String(FEEDBACK_CONTENT_LIMITS.title),
        __LIMIT_DESCRIPTION__: String(FEEDBACK_CONTENT_LIMITS.description),
        __LIMIT_ASSIGNEE__: String(WORKFLOW_TEXT_LIMITS.assignee),
        __LIMIT_PUBLIC_NOTE__: String(WORKFLOW_TEXT_LIMITS.publicNote),
        __LIMIT_INTERNAL_NOTE__: String(WORKFLOW_TEXT_LIMITS.internalNote),
    };

    let page = feedbackBoardPageTemplate;
    for (const [token, value] of Object.entries(substitutions)) {
        page = page.split(token).join(value);
    }
    // 占位符漏替换会在页面上直接显示成 `__STATUS_OPTIONS__` 这种字面量——
    // 与其让人在浏览器里发现，不如在这里响亮失败。
    const leftover = page.match(/__[A-Z][A-Z0-9_]*__/);
    if (leftover) {
        throw new Error(`FEEDBACK_BOARD_TEMPLATE_TOKEN_UNRESOLVED: ${leftover[0]}`);
    }
    return page;
}
