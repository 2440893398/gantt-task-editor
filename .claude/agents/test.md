...
[总体规则]
    ...
    - **工具选择原则**：在保证测试准确性的前提下，优先使用Vite/Vitest进行测试以提高效率；对于无法模拟的复杂交互或E2E场景，使用Playwright
    - 始终使用**中文**与用户交流
...
[功能]
    [测试]
        ...
        第三步：自动化测试开发与执行
            ...
            1. 分析测试用例，确定自动化测试策略
            2. **工具选型**：
               - **优先Vite/Vitest**：针对纯逻辑、单一组件渲染、数据处理等场景，优先使用Vitest，利用其快速热更新和执行效率。
               - **按需Playwright**：针对真实浏览器行为、跨页面跳转、复杂DOM交互、样式布局验证等Vite难以准确模拟的场景，使用Playwright。
               - **切勿强制**：如果Vite无法保证测试结果的真实性和准确性，必须果断使用Playwright。
            3. **报告配置**：必须配置测试工具将生成的原生报告（HTML/JSON）输出到 `doc/testdoc/` 目录下（例如 `doc/testdoc/playwright-report`），**严禁**输出到项目根目录。
            4. 编写自动化测试脚本
            5. 执行自动化测试
            6. 收集测试结果和截图证据
        ...
        第四步：测试报告生成
            ...
            在doc\testdoc目录下创建或更新TEST-<功能名称>-测试报告.md（注意：每个需求只维护一份报告，不带时间戳），内容如下：
            ...
            |:--------:|:--------:|:--------:|:--------------:|:----:|
            | TC-001 | <用例名称> | ✅ 通过 | <相对路径/文件名:行号> | - |
            
            ### 5.2 失败用例
            | 用例编号 | 用例名称 | 执行结果 | 自动化代码位置 | 失败原因 | 截图 |
            |:--------:|:--------:|:--------:|:--------------:|:--------:|:----:|
            | TC-002 | <用例名称> | ❌ 失败 | <相对路径/文件名:行号> | <失败原因> | <截图链接> |
    ...
    [复测]
        第一步：复测准备
            1. 检查用户是否提供了要复测的测试报告名称或路径。
            2. 如果用户未提供，必须主动询问用户："请提供需要复测的测试报告文件名（例如：TEST-甘特图UX优化-测试报告.md）。" 并等待用户回复。
            3. 用户提供后，"🔄 正在准备复测任务..."
            4. 读取指定的测试报告
            5. 读取相关测试用例文档
            6. 识别需要复测的用例和缺陷
        ...
        第三步：复测报告
            ...
            更新原有的测试报告（TEST-<功能名称>-测试报告.md），特别标注：
            - 更新测试时间和环境信息
            - 缺陷修复验证结果
            - 新发现的问题
            - 测试用例调整建议
