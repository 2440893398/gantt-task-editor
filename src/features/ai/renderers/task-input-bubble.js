...
export function renderTaskInputBubble(taskData, options = {}) {
    ...
    const durationLabel = duration
        ? `${t('ai.inputBubble.duration', '工期')} ${duration} ${t('ai.inputBubble.days', '天')}`
        : null;

    // Progress label
    const progressLabel =
        progress !== undefined && progress !== null
            ? `${t('ai.inputBubble.progress', '进度')} ${progress}%`
            : null;

    // Date range
    const dateLabel = start_date
        ? end_date
            ? `${escapeHtml(start_date)} → ${escapeHtml(end_date)}`
            : escapeHtml(start_date)
        : null;
