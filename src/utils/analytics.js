...
export const Events = {
    ...
    taskCreate: (source = 'toolbar') => trackEvent('task_create', { source }),

    /** User edited a task */
    taskEdit: (field) => trackEvent('task_edit', { field }),

    /** User exported data */
    exportFile: (format) => trackEvent('export_file', { format }),

    /** User imported data */
    importFile: (format, rowCount) => trackEvent('import_file', { format, row_count: rowCount }),

    /** AI chat interaction */
    aiChat: (skillType) => trackEvent('ai_chat', { skill_type: skillType }),

    /** Language switch */
    languageSwitch: (language) => trackEvent('language_switch', { language }),

    /** View mode switch (split/table/gantt) */
    viewModeSwitch: (mode) => trackEvent('view_mode_switch', { mode }),

    /** Baseline saved */
    baselineSave: (taskCount) => trackEvent('baseline_save', { task_count: taskCount }),

    /** Critical path toggled */
    criticalPathToggle: (enabled) => trackEvent('critical_path_toggle', { enabled }),

    /** Batch edit applied */
    batchEdit: (field, taskCount) => trackEvent('batch_edit', { field, task_count: taskCount }),
};
