...
7. **验证层级不得下沉**：依赖真实布局的行为——滚动位置、居中/对齐、元素几何、可见性、
   命中区——验证点只认浏览器级测试（Playwright），不接受 jsdom 单测替代。jsdom 里这些量
   要么恒为 0，要么来自 mock，断言的其实是 mock 值本身。反例（真实发生过）：某次"点击今天
   居中"的修复在浏览器里实测偏差 356px，改用单测并把 `getScrollState` mock 成 `{ x: 0 }`
   后立刻转绿——而 `{ x: 0 }` 正是那个 bug 唯一不显形的取值。已有浏览器级验证点不得被改写
   为单测；确需改测试层级的，按规则 1 当作契约变更处理。
