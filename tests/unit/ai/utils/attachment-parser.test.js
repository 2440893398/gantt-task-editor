...
describe('attachment parser', () => {
    it('returns warning for docx attachments', async () => {
        const { parseAiAttachmentFile } =
            await import('../../../../src/features/ai/utils/attachment-parser.js');
        const file = {
            name: 'spec.docx',
            size: 2048,
    ...
    it('parses excel file into bounded prompt context', async () => {
        const { parseAiAttachmentFile, __test__ } =
            await import('../../../../src/features/ai/utils/attachment-parser.js');
        const file = {
        ...
        expect(result.attachmentContext.promptBlock.length).toBeLessThanOrEqual(
            __test__.LIMITS.maxPromptChars
        );
