    ...
    test.describe('6.3 Shortcuts Panel Localization - 快捷键面板本地化', () => {
        ...
        test('TC-L-034: Navigation section title - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                const navTitle = await page
                    .locator('[data-i18n="shortcuts.navigation"]')
                    .textContent();
            ...
            if (failures.length > 0) {
                const failMsg = failures
                    .map((f) => `${f.lang}: expected "${f.expected}", got "${f.actual}"`)
                    .join('; ');
        ...
        test('TC-L-035-040: Navigation shortcuts labels - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                const panViewText = await page
                    .locator('[data-i18n="shortcuts.panView"]')
                    .textContent();
                const zoomTimelineText = await page
                    .locator('[data-i18n="shortcuts.zoomTimeline"]')
                    .textContent();
                const goToTodayText = await page
                    .locator('[data-i18n="shortcuts.goToToday"]')
                    .textContent();
        ...
        test('TC-L-041-047: Task operations labels - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                const taskOpsText = await page
                    .locator('[data-i18n="shortcuts.taskOperations"]')
                    .textContent();
                const expected = LOCALIZATION_MAP[lang];
                ...
                console.log(
                    `[TC-L-041] ${lang}: TaskOperations - ${hasTaskOperations ? 'FOUND' : 'NOT FOUND'}`
                );
        ...
        test('TC-L-050-053: Legend labels - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                const completedText = await page
                    .locator('[data-i18n="shortcuts.completed"]')
                    .textContent();
                const incompleteText = await page
                    .locator('[data-i18n="shortcuts.incomplete"]')
                    .textContent();
                const dependencyText = await page
                    .locator('[data-i18n="shortcuts.dependency"]')
                    .textContent();
                const expected = LOCALIZATION_MAP[lang];
    ...
    test.describe('6.2 View Selector Localization - 视图选择器本地化', () => {
        test('TC-L-001-005: View selector options - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                console.log(
                    `  Expected: Day="${expected.dayView}", Week="${expected.weekView}", Month="${expected.monthView}"`
                );
                ...
                if (lang !== 'zh-CN') {
                    ...
                    if (hasChineseOptions) {
                        console.log(
                            `[LOCALIZATION BUG] ${lang} view selector still contains Chinese options:`,
                            options
                        );
                    }
                }
    ...
    test.describe('6.5 Toolbar Buttons Localization - 工具栏按钮本地化', () => {
        test('TC-L-060: Today button - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                console.log(
                    `[TC-L-060] ${lang}: Today button - Expected "${expected}", Actual "${todayBtnText.trim()}"`
                );
        ...
        test('TC-L-061: New Task button - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                const newTaskText = await page
                    .locator('#new-task-btn span[data-i18n="toolbar.newTask"]')
                    .textContent();
                const expected = LOCALIZATION_MAP[lang].newTask;

                console.log(
                    `[TC-L-061] ${lang}: New Task button - Expected "${expected}", Actual "${newTaskText.trim()}"`
                );
        ...
        test('TC-L-062: More button - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                console.log(
                    `[TC-L-062] ${lang}: More button - Expected "${expected}", Actual "${moreText.trim()}"`
                );
    ...
    test.describe('6.6 Dropdown Menu Localization - 下拉菜单本地化', () => {
        test('TC-L-070-074: Dropdown menu items - All languages', async ({ page }) => {
            ...
            for (const lang of languages) {
                ...
                // 获取菜单项文本
                const exportExcelText = await page
                    .locator('#dropdown-export-excel span[data-i18n="toolbar.exportExcel"]')
                    .textContent();
                const importExcelText = await page
                    .locator('#dropdown-import-excel span[data-i18n="toolbar.importExcel"]')
                    .textContent();
                const exportJsonText = await page
                    .locator('#dropdown-export-json span[data-i18n="toolbar.exportJSON"]')
                    .textContent();
                const importJsonText = await page
                    .locator('#dropdown-import-json span[data-i18n="toolbar.importJSON"]')
                    .textContent();
                const languageText = await page
                    .locator('#language-menu span[data-i18n="toolbar.language"]')
                    .textContent();
    ...
    test.describe('6.9 Language Switch Edge Cases - 语言切换边界测试', () => {
        ...
        test.skip('TC-L-104: Batch edit panel localization', async ({ page }) => {
            ...
            const panelTitle = await page
                .locator('#batch-edit-panel h3[data-i18n="batchEdit.title"]')
                .textContent();
        ...
        test.skip('TC-L-105: Field management panel localization', async ({ page }) => {
            ...
            // 检查面板标题
            const panelTitle = await page
                .locator('#field-management-panel h4[data-i18n="fieldManagement.title"]')
                .textContent();
