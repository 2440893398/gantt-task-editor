...
describe('calendarTools', () => {
    ...
    describe('get_calendar_info', () => {
        it('returns calendar summary with deterministic totals', async () => {
            ...
            expect(result.holidays[0]).toMatchObject({
                date: '2026-05-01',
                country_code: 'JP',
                is_off_day: true,
            });
            expect(result.custom_days[0]).toMatchObject({
                id: 'c-1',
                date: '2026-05-10',
                is_off_day: false,
            });
    ...
    describe('get_assignee_workload', () => {
        ...
        it('aggregates workload by assignee and supports filters', async () => {
            ...
            expect(allResult.totals).toEqual({
                assignee_count: 2,
                total_tasks: 3,
                total_duration: 10,
            });
            ...
            const filtered = await calendarTools.get_assignee_workload.execute({
                assignee: 'Alice',
                status: 'pending',
            });
